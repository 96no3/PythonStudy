...
class Frame(Tk.Frame):
    def __init__(self, master, m, **key):
        ...
        self.echo.set('%02d:%02d' % (self.min, self.sec%60))
        ...
        display.bind('<1>', self.start_stop)
        display.bind('<3>', self.reset)
...
if __name__ == '__main__':
    m = len(sys.argv) > 1 and  int(sys.argv[1]) or 3
    f = Frame(None,m)
    f.pack()
    f.mainloop()
