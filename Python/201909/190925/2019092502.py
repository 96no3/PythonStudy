...
class Node:
    ...
    def parse(self):
        ...
        def parse_inorder():            
            pos_operator = check_pos_operator()
        ...
        def parse_postorder():
            if self.expression == []:
                self.expression = [tree.pop()]
                return
            elif _operators.count(self.expression[0]) == 1:
                self.right = Node([tree.pop()])
                self.left = Node([tree.pop()])
                tree.push(self.expression.pop())
                self.parse()
            elif validate_num(self.expression[0]):
                tree.push(self.expression.pop())
                self.parse()
