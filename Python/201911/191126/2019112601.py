...
static1 = tk.Label(root,text=u"▼Entry▼",foreground="#ff0000",background="#000000")
static1.pack()
#static1.place(x=0,y=150) 
 
# Entry出現
#Entry1 = tk.Entry()
entry1 = tk.Entry(root,width=50) # widthプロパティで大きさを変える
...
# Buttonを設置
#button1 = tk.Button(root,text=u"button1")
#button1 = tk.Button(root,text=u"Elase",width=50)
 
# 関数に引数を渡す場合は、commandオプションとlambda式を使う
#button1 = tk.Button(root,text=u"Message Button",width=50,command=lambda:showMessage(entry1.get()))
button1 = tk.Button(root,text=u"追加するButton",width=50,command=lambda:addList(entry1.get()))
 
#button1.bind("<Button-1>",deleteEntry) # buttonが押されたときに実行される関数をバインド
button1.pack()
 
button2 = tk.Button(root,text=u"削除するButton",width=50,command=lambda:deleteSelectedList())
button2.pack()
 
 
# リストボックスを設置
#listbox1 = tk.Listbox(root,)
listbox1 = tk.Listbox(root,width=55,height=14)
listbox1.pack()
